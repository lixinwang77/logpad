...
- **套用预设**（追加语义、自动去重）：
  ...
  - 点击**单个过滤词**：把该词**追加**到搜索框（属显式操作，不受勾选状态影响）
  - 追加/合并时按 `|` 拆分**去重**，重复套用同一组/词不会叠加
...
- [x] 点击组名套用组内**已勾选**的词（`|` 合并）、点击单词套用单词，均追加到搜索框（按 `|` 去重）并自动开启 Regex，触发搜索与自动分屏
- [x] 每个过滤词带启用勾选框，仅勾选的词参与套用整组，新词默认勾选，状态持久化
