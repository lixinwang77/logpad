...
- **过滤词启用开关**：每个过滤词左侧有勾选框（`checkmark.square.fill` / `square`）。仅**勾选**的词会在「套用整组」时生效；**取消勾选**的词在套用整组时被排除（行内文字置灰）。新添加的词默认勾选。该状态随预设持久化
- **点击过滤词** = 切换该词的勾选/取消勾选（与点勾选框等效）；点击过滤词**不会**直接把词加入搜索框，套用统一通过点击组名进行
- **套用预设**（追加语义、自动去重）：
  - 点击**组名**：对本组在搜索框中的贡献做**同步**——先移除搜索框里属于本组的全部词，再加入本组当前**已勾选**的词（用 `|` 合并）；其它来源的词（手输或别的组）保持不动。因此取消勾选某词后再次套用，搜索框会**去掉**该词
  - 点击**单个过滤词**：仅切换该词的勾选状态，不直接套用
  - 合并时按 `|` 拆分**去重**，重复套用同一组不会叠加
...
struct FilterPresetWord: Codable, Identifiable, Equatable {
    ...
    var isEnabled: Bool = true    // 是否参与「套用整组」（缺字段时向后兼容默认 true）
}
...
- [x] 点击组名套用组内**已勾选**的词（`|` 合并、对本组同步、按 `|` 去重）并自动开启 Regex，触发搜索与自动分屏
- [x] 每个过滤词带启用勾选框，点击勾选框或词文字均切换勾选；仅勾选的词参与套用整组，新词默认勾选，状态持久化
